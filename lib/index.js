...
// SInce polylineShortcuts is array[0..180][0..360]
// lets use these funtion to get polyline shortcut index
var latIndex = function(lat) {
  return Math.floor(lat) + 90
}

var lonIndex = function(lon) {
  return Math.floor(lon) + 180
}

var getShortcuts = function(lat,lon) {
  var latArray = polyPointsShortcuts[ latIndex(lat) ]
  if(latArray) return latArray[ lonIndex(lon) ]
}  
 
var addPolyPointShortcut = function (tzname, pointObj) {
  var latFloor = latIndex(pointObj.lat);
  var lngFloor = lonIndex(pointObj.lng);
...
var searchNeighbours = function(latFl, lonFl, maxDist) {
  ...
  var y = Math.floor(latFl), x = Math.floor(lonFl);
  ...
  for (var i = 0; i < maxRectSides; i++) {
    ...
    if( foundShortcut = getShortcuts(y,x) ) return foundShortcut;
    }
...
//     .distance  Distance from input coords in degrees
//     .latitude  Point lat.
//     .longitude Point lon.
var tzNameNear = function (lat,lon, maxDistance) {
  var maxDistance = maxDistance || 2
  , points = getShortcuts(lat,lon) || searchNeighbours(lat, lon, maxDistance)
  , minTz 
  , minDist = Infinity;

  // We create list of timezones and nearest points
  // the we select timezone with nearest point
  for (var tzname in points) {

    var near = geolib.findNearest({'lat': lat, 'lng': lon}, points[tzname])
    , dist = near.distance;
